...
router\Router::route('/test', function(){
  ...
  //  $obj = new \classes\Account();
 //   $resu = $obj->verifyMailUpdatePass();


router\Router::route('/(/updatepass)/(\d+)', function($category, $id){  
echo '/(\w+)/(\d+)'.$category.$id;
    echo "тута";
    die();
  //  $obj = new \classes\Account();
 //   $resu = $obj->verifyMailUpdatePass();
...
router\Router::route('/tools', function(){  
 //   $obj = new \classes\Category;
 //   $obj1 = new \classes\Admin();
 //  $resu = $obj->cat();
 //  $resu2 = $obj1->catId();
